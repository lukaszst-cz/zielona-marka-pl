...
export const metadata: Metadata = {
  ...
  description: "Strony, formularze wyceny i asystenty dla warsztatów, ekip remontowych, instalatorów, branży beauty i firm usługowych z Marek oraz okolic.",
...
const problems = [
  ...
  ["04", "Te same pytania zabierają czas", "Asystent odpowiada z zatwierdzonej bazy wiedzy, zbiera kontakt i przekazuje rozmowę człowiekowi."],
];
  ...
  return <>
    ...
    <main id="top">
      <section className="sales-hero shell">
        <div className="eyebrow"><span />STRONY WWW · FORMULARZE WYCENY · ASYSTENTY</div>
      ...
      <section className="problem-section"><div className="shell">
        ...
        <div className="problem-grid">{problems.map(([number, title, copy]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p><Link href={number === "02" ? "/modernizacja-strony" : number === "04" ? "/asystent-zapytan" : "/oferta"}>Zobacz rozwiązanie <b>↗</b></Link></article>)}</div>
      </div></section>
      ...
      <section className="section shell">
        ...
        <div className="project-preview-grid">{projects.filter(project => project.name === "Auto Naprawa" || project.name === "Natura Studio" || project.name === "TransportFlow" || project.name === "DetailFlow").map((project) => <article key={project.name} className="project-preview"><div className="project-preview-image" style={{ backgroundImage: `linear-gradient(180deg,rgba(10,31,22,.08),rgba(10,31,22,.78)),url(${project.imageUrl})` }}><span>{project.note}</span><b>{project.n}</b></div><div><small>{project.type}</small><h3>{project.name}</h3><p>{project.description}</p><Link href={project.websiteUrl.startsWith("/") ? project.websiteUrl : "/realizacje"}>Zobacz demonstrację <b>↗</b></Link></div></article>)}</div>
      ...
      <section className="asystent-band dark-section"><div className="shell asystent-band-grid"><div><span className="section-no">ASYSTENT DEMONSTRACYJNY</span><h2>Najpierw wypróbuj go <em>na naszej stronie.</em></h2><p>Asystent w prawym dolnym rogu pokaże, jak rozpoznać potrzebę klienta, zebrać kontakt i przekazać uporządkowane zgłoszenie.</p></div><div><button className="button button-light" type="button" data-open-demo-kontakt>Uruchom asystenta <span>↗</span></button><Link href="/asystent-zapytan">Jak działa wdrożenie <span>→</span></Link></div></div></section>
