...
export default function ImprovementsPage() {
  return <><SiteHeader /><main>
    ...
    <section className="section shell"><div className="section-head"><div><span className="section-no">ZACZYNAMY OD KONKRETU</span><h2>Prosty panel zleceń zamiast chaosu w wiadomościach.</h2></div><p>To może być mały CRM, czyli prosty system dopasowany do Twojej firmy. Zamiast kupować duży program, zaczynamy od klientów, zleceń, statusów i kolejnego kroku.</p></div><div className="situation-grid">{situations.map(([title, text], index) => <article key={title}><b>0{index + 1}</b><h3>{title}</h3><p>{text}</p></article>)}</div></section>
    ...
    <section className="section shell"><div className="section-head"><div><span className="section-no">MOŻLIWE KLOCKI</span><h2>Dobierane wtedy, gdy są potrzebne.</h2></div><p>Możesz zacząć od jednego przepływu. Jeśli działa, spokojnie rozbudujemy go w kolejnym etapie.</p></div><div className="tool-grid">{tools.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
