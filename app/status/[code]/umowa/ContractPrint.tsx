...
export default function ContractPrint({ project: p }: { project: Project }) {
  return <main className="contract-page">
    ...
    <article>
      ...
      <section><h2>§ 1. Strony umowy</h2><p><b>Wykonawca:</b> {value(p.providerName)}, adres: {value(p.providerAddress)}, NIP: {value(p.providerNip)}, e-mail: lukasz.staniewicz@gmail.com.</p><p><b>Zamawiający:</b> {value(p.clientCompany || p.clientName)}, reprezentowany przez: {value(p.clientName)}, adres: {value(p.clientAddress)}, NIP: {value(p.clientNip)}, e-mail: {value(p.clientEmail)}.</p></section>
      ...
      <section><h2>§ 4. Wynagrodzenie</h2><p>Łączne wynagrodzenie za uzgodniony zakres wynosi <b>{p.price ? p.price.toLocaleString("pl-PL") + " zł netto" : "DO UZUPEŁNIENIA"}</b>. Harmonogram płatności i dodatkowe koszty wymagają wpisania przed podpisaniem: ............................................................</p></section>
